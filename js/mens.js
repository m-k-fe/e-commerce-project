...
let getMensProducts = JSON.parse(localStorage.getItem("mens-products"));
...
drawProductsInUi(getMensProducts);
let select = document.querySelector("#select");
select.addEventListener("change",function(e){
    if (e.target.value === "name"){
        let myArr = getMensProducts.map(item => item.name).sort();
        let arr = [];
        myArr.forEach(item => {
            let obj = getMensProducts.find(el => el.name === item);
            arr.push(obj);
        })
        productsDom.innerHTML = "";
        drawProductsInUi(arr);
    }else if (e.target.value === "price"){
        let arr = getMensProducts.sort((a,b) => +a.newPrice.slice(1) - +b.newPrice.slice(1));
        productsDom.innerHTML = "";
        drawProductsInUi(arr);
    }else{
        productsDom.innerHTML = "";
        drawProductsInUi(JSON.parse(localStorage.getItem("mens-products")));
    }
})
...
inputsRadio.forEach(item => {
    item.addEventListener("change",function(e){
        if (e.target.id === "large"){
            getProductsBySize("mens","large");
        }else if (e.target.id === "medium"){
            getProductsBySize("mens","medium");
        }else if (e.target.id === "small"){
            getProductsBySize("mens","small");
        }else{
            productsDom.innerHTML = "";
            drawProductsInUi(JSON.parse(localStorage.getItem("mens-products")));
        }
...
inputSearch.addEventListener("keyup",function(e){
    searchByName(JSON.parse(localStorage.getItem("mens-products")),e.target.value);
    if (e.target.value === ""){
        productsDom.innerHTML = "";
        drawProductsInUi(JSON.parse(localStorage.getItem("mens-products")));
    }
